...
local setmetatable, type, select, xpcall, pcall, pairs, tinsert, tremove =
  setmetatable, type, select, xpcall, pcall, pairs, tinsert, tremove
local EventRegistry, MergeTable, CopyTable, CallErrorHandler =
  EventRegistry, MergeTable, CopyTable, CallErrorHandler
local Embeds = {}

local Closures = setmetatable({}, {
...
local function safecall(object, key, ...)
  ...
  if type(func) == "function" then xpcall(func, CallErrorHandler, object, ...) end
end

local function embed(object, ...)
  for i = 1, select("#", ...) do
    local lib = select(i, ...)
    if type(lib) == "string" then lib = LibStub(lib) end

    if type(lib) == "table" then safecall(lib, "Embed", object) end
  end
  return object
end
...
local function newobject(domain, id, object, ...)
  ...
  if domain then domain[id] = object end
...
Lib.Calls = Lib.Calls
  or LibStub("CallbackHandler-1.0"):New(Lib, "Register", "Unregister", "UnregisterAll")
...
function Embeds:UnregisterSignal(event) Lib.Unregister(self, self.Tag .. event) end

function Embeds:SendSignal(event, ...)
  event = self.Tag .. event
  Lib.Calls:Fire(event, ...)

  if EventTrace then EventTrace:LogCallbackRegistryEvent("WildAddon", event, ...) end
end
...
Embeds.None = setmetatable(
  {},
  { __newindex = function() error("None table is immutable.", 2) end, __metatable = false }
)
...
function Lib:Embed(object) MergeTable(object, Embeds) end
