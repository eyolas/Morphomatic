...
function Base:NewSushi(name, version, type, template, global)
  local class = Lib[name]
    or self:NewClass(type, (global or self:GetClassName()) and ("Sushi-3.2-" .. name), template)
  local old = rawget(class, "__version") or 0
  if old >= version then return end

  for k, v in pairs(class) do
    if k:sub(1, 2) ~= "__" then class[k] = nil end
  end

  setmetatable(class, Meta)
  for k, v in pairs(class.__base) do
    if not class.__super[k] then class[k] = v end
  end
...
function Meta:__newindex(key, value)
  rawset(
    ...
    key,
    type(value) == "function"
        and (key:sub(1, 3) == "Set" or key:sub(1, 4) == "Fire" or key == "Show" or key == "Hide")
        and chain(value)
      or value
  )
end
