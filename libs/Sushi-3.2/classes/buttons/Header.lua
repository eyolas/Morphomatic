...
function Header:New(parent, text, font, underlined)
  ...
  if parent.SetCall then parent:SetCall("OnResize", function() b:UpdateWidth() end) end
...
function Header:OnEnter()
  ...
  self:GetFontString():SetText(self:GetText():gsub("|c(" .. strrep("%x", 8) .. ")", function(value)
    return "|c"
      .. value:gsub(
        "(%x%x)",
        function(v) return format("%02x", min(255, tonumber(v, 16) * self:GetHighlightFactor())) end
      )
  end))
end
...
function Header:GetText() return self.text end
...
function Header:IsUnderlined() return self.Line:IsShown() end

function Header:SetHighlightFactor(factor) self.highlight = factor end

function Header:GetHighlightFactor() return self.highlight end

function Header:SetJustifyH(justify) self:GetFontString():SetJustifyH(justify) end

function Header:GetJustifyH() return self:GetFontString():GetJustifyH() end
