...
function Group:Add(object, ...)
  ...
  assert(
    kind == "string" or kind == "function" or kind == "table",
    "Bad argument #1 to `:Add` (string, function or frame expected)"
  )
  ...
  if object.SetCall then
    object:SetCall("OnUpdate", function()
      self:Update()
...
function Group:AddBreak() tinsert(self.Children, self.Break) end
...
function Group:IterateChildren() return ipairs(self.Children) end

function Group:NumChildren() return #self.Children end
...
function Group:Layout(force)
  ...
  for i, child in self:IterateChildren() do
    if child ~= self.Break then
      ...
      if self.limit and (x + width) > self.limit then breakLine() end
...
function Group:CanLayout() return self:GetCalls("OnChildren") and self:IsVisible() end
...
function Group:GetOrientation() return self.orientation end

function Group:Orient(a, b)
  if self:GetOrientation() == "HORIZONTAL" then return a, b end
  return b, a
end
...
function Group:GetResizing() return self.resizing end
...
function Group:SetBackdrop(template)
  if self.pool then self.pool:Release(self.bg) end
...
function Group:GetBackdrop() return self.backdrop end
