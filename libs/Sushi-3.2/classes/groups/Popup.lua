...
local Magnifier = WOW_PROJECT_ID == WOW_PROJECT_MAINLINE
    and "communities-icon-searchmagnifyingglass"
  or "shop-games-magnifyingglass"
...
local function lastPopup()
  ...
  while true do --> thanks blizz, very simple and efficient, great change making everything local
    ...
    if not frame or not frame:IsShown() then return _G["StaticPopup" .. i] end
...
function Popup:Toggle(input) return self:Cancel(input) or self:New(input) end
...
function Popup:GetActive(target)
  for i, f in self:IterateActive() do
    if f == target or f.id == target then return f, i end
  end
end

function Popup:IterateActive() return ipairs(self.Active) end
...
function Popup:New(input)
  ...
  f.button1, f.button2, f.moneyInput, f.hideOnEscape =
    info.button1, info.button2, info.moneyInput, f.hideOnEscape
  f.text = (info.text or "")
    .. (not f.moneyInput and f.money and ("|n" .. GetCoinTextureString(f.money)) or "")
...
function Popup:Populate()
  if self.text then self:Add("Header", self.text, GameFontHighlightLeft):SetJustifyH("CENTER") end

  if self.edit then
    self.textInput = self
      :Add("BoxEdit", nil, self.edit)
      :SetWidth(240)
      :SetKeys({ centered = true, top = -6, left = -3, right = -3, bottom = -3 })
  ...
  if self.button1 or self.button2 then
    local buttons = self:Add("Group", function(group)
      if self.button1 then
        group
          :Add("RedButton", self.button1)
          :SetSize(118, 20)
          :SetCall("OnClick", function() self:Accept() end).right =
          0
      end

      if self.button2 then
        group
          :Add("RedButton", self.button2)
          :SetSize(118, 20)
          :SetCall("OnClick", function() self:Release("closed") end).right =
          0
      end
    end)
...
function Popup:Release(reason)
  ...
  if i then
    ...
    if reason then self:FireCalls("OnCancel", reason) end
