...
function Choice:GetValue() return self.value end
  ...
  if type(key) == "table" then
    ...
    if data.key then
      ...
      for i, choice in ipairs(data) do
        if type(choice) == "table" and choice.key then tinsert(self.choices, choice) end
      end
    end
  elseif key then
...
function Choice:GetChoices() return self.choices end

function Choice:UpdateText()
  for i, choice in ipairs(self:GetChoices()) do
    if choice.key == self:GetValue() then return self.Text:SetText(choice.text or choice.key) end
  end
