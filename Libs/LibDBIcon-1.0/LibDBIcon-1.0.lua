...
local next, Minimap, CreateFrame, AddonCompartmentFrame =
  next, Minimap, CreateFrame, AddonCompartmentFrame
lib.tooltip = lib.tooltip
  or CreateFrame("GameTooltip", "LibDBIconTooltip", UIParent, "GameTooltipTemplate")
...
local function onLeave(self)
  ...
  if not isDraggingButton then
    for _, button in next, lib.objects do
      if button.showOnMouseover then button.fadeOut:Play() end
    end
  end
...
local function onLeaveCompartment(self, menu)
  ...
  local object = lib.objects[buttonName]
  if object and object.dataObject then
    if object.dataObject.OnLeave then object.dataObject.OnLeave(self) end
  end
end
...
function lib:Register(name, object, db, customCompartmentIcon)
  ...
  if lib:GetMinimapButton(name) then
...
function lib:Lock(name)
  ...
  if button then
    ...
    if button.db then button.db.lock = true end
  end
end
...
function lib:Unlock(name)
  ...
  if button then
    button:SetScript("OnDragStart", onDragStart)
    button:SetScript("OnDragStop", onDragStop)
    if button.db then button.db.lock = nil end
  end
end
...
function lib:IsButtonCompartmentAvailable()
  if AddonCompartmentFrame then return true end
end

function lib:IsButtonInCompartment(buttonName)
  local object = lib.objects[buttonName]
  if object and object.db and object.db.showInCompartment then return true end
  return false
end
    ...
    if object and object.compartmentData then
      for i = 1, #AddonCompartmentFrame.registeredAddons do
        ...
        if entry == object.compartmentData then
          ...
          if object.db then object.db.showInCompartment = nil end
