...
local function buildListForUI(filter)
  ...
  if not filter or filter == "" then return base end
...
local function buildCanvasMacro()
  ...
  auto:SetScript(
    "OnClick",
    function(selfBtn) MM.DB:Get().autoCreateMacro = selfBtn:GetValue() and true or false end
  )
...
local function buildCanvasFavorites()
  ...
  skipcd:SetScript(
    "OnClick",
    function(selfBtn) MM.DB:Get().skipOnCooldown = selfBtn:GetValue() and true or false end
  )
...
local function GetLastCategory() return Options._lastOptionsCat or "main" end
...
function Options:Open(which) -- public
  ...
  if Blz and Blz.OpenToCategory and id then Blz.OpenToCategory(id) end
end
