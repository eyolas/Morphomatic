-- Optional floating button (secure-triggered via macrotext)
MM = MM or {}
...
-- Create the floating button (SecureActionButtonTemplate) that runs the same macro flow
function MM.CreateFloatingButton()
  ...
  floatBtn:SetScript("OnDragStart", function(s) if not MM.DB().button.locked then s:StartMoving() end end)
  floatBtn:SetScript("OnDragStop",  function(s) s:StopMovingOrSizing(); saveAnchor(s) end)

  -- Make it execute the same macro as the created 'MM' macro:
  local macrotext = "/run if MM and MM.PrepareSecureUse then MM.PrepareSecureUse() end\n/click MM_SecureUse"
  ...
  tex:SetAllPoints()
  tex:SetTexture(134414) -- generic toy-ish icon
  floatBtn.icon = tex
  ...
  floatBtn:SetScript("OnEnter", function(self)
    ...
    tip:AddLine("Click: random cosmetic toy (from your selection).", 0.9,0.9,0.9, true)
    tip:AddLine("Drag to move (if unlocked).", 0.7,0.7,0.7)
    tip:Show()
  end)
...
function MM.ResetButtonAnchor()
  ...
  if MM_Float then MM_Float:ClearAllPoints(); MM_Float:SetPoint("CENTER", UIParent, "CENTER", 0, 0) end
end
